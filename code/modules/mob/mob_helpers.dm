		...
		if(M.client && ((!istype(M, /mob/new_player) && M.stat == DEAD) || (M.client.holder && !is_mentor(M.client))) && (M.client.prefs.toggles & CHAT_DEAD))
			...
			if(subject)
				...
				if(M.client.holder) 							// What admins see
					lname = "[keyname][(DM && (DM.client.prefs.toggles & CHAT_GHOSTEARS)) ? "*" : (DM ? "" : "^")] ([name])"
				else
					if(DM && (DM.client.prefs.toggles & CHAT_GHOSTEARS))	// If the person is actually observer they have the option to be anonymous
						lname = "Ghost of [name]"
			...
			M << "<span class='deadsay'>DEAD: [lname][follow][message]</span>"
