datum/preferences
	...
	proc/update_preview_icon()		//seriously. This is horrendous.
		...
		var/body = body_build?"[body_build]":""
		...
		preview_icon.Blend(new /icon(icobase, "groin_[g][body]"), ICON_OVERLAY)
		preview_icon.Blend(new /icon(icobase, "head_[g][body]"), ICON_OVERLAY)
		...
		for(var/name in list("r_arm","r_hand","r_leg","r_foot","l_leg","l_foot","l_arm","l_hand"))
			...
			if(organ_data[name] == "cyborg")
				temp = new /icon('icons/mob/human_races/robotic.dmi', "[name]_[g][body]")
			else
				temp = new /icon(icobase, "[name]_[g][body]")
		...
		if(underwear && current_species.flags & HAS_UNDERWEAR)
			underwear_s = new/icon("icon" = 'icons/mob/human.dmi', "icon_state" = "[underwear]_[g][body]")

		var/icon/undershirt_s = null
		if(undershirt && current_species.flags & HAS_UNDERWEAR)
			undershirt_s = new/icon("icon" = 'icons/mob/human.dmi', "icon_state" = "[undershirt]_[g][body]")
		...
		if(job_civilian_low & ASSISTANT)//This gives the preview icon clothes depending on which job(if any) is set to 'high'
			...
			if(backbag == 2)
				clothes_s.Blend(new /icon((g == "f" && body)?'icons/mob/back_f.dmi':'icons/mob/back.dmi', "backpack"), ICON_OVERLAY)
			else if(backbag == 3 || backbag == 4)
				clothes_s.Blend(new /icon((g == "f" && body)?'icons/mob/back_f.dmi':'icons/mob/back.dmi', "satchel"), ICON_OVERLAY)
			...
			if(J)//I hate how this looks, but there's no reason to go through this switch if it's empty
				...
				if(GL) clothes_s.Blend(new /icon((g == "f" && body)?'icons/mob/hands_f.dmi':'icons/mob/hands.dmi', initial(GL.item_state)), ICON_UNDERLAY)

				var/obj/item/weapon/storage/belt/BT = J.belt
				if(BT) clothes_s.Blend(new /icon((g == "f" && body)?'icons/mob/belt_f.dmi':'icons/mob/belt.dmi', initial(BT.item_state)), ICON_OVERLAY)

				var/obj/item/clothing/suit/ST = J.suit
				if(ST) clothes_s.Blend(new /icon((g == "f" && body)?'icons/mob/suit_f.dmi':'icons/mob/suit.dmi', initial(ST.item_state)), ICON_OVERLAY)
				...
				if( backbag > 1 )
					...
					clothes_s.Blend(new /icon((g == "f" && body)?'icons/mob/back_f.dmi':'icons/mob/back.dmi', initial(BP.icon_state)), ICON_OVERLAY)

		if(disabilities & NEARSIGHTED)
			preview_icon.Blend(new /icon((g == "f" && body)?'icons/mob/eyes_f.dmi':'icons/mob/eyes.dmi', "glasses"), ICON_OVERLAY)
