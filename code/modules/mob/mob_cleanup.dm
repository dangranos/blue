...
/mob/proc/contract_disease(var/datum/disease/virus, var/skip_this = 0, var/force_species_check=1, var/spread_type = -5)
	...
	if(stat >=2)	//world << "He's dead jim."
		return
	if(istype(virus, /datum/disease/advance))	//world << "It's an advance virus."
...
/mob/living/carbon/human/contract_disease(var/datum/disease/virus, var/skip_this = 0, var/force_species_check=1, var/spread_type = -5)
	if(species.name in virus.immune_species) return
	..()